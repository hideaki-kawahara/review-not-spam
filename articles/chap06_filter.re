...
==={HTML} HTMLメール規制（au）
HTMLメールは画像付きのメールみたいなものです。文字情報以外も表示されるので広告宣伝メールに最適ですが迷惑メールでは非常に多いです。HTMLメールにすることでURLを隠すことができるので、フィッシング詐欺によく使われます。また、HTMLメールは画像を外部サイトから表示するので、業者がメール受信者ごとにユニークなURLにしてメール受信者の存在確認をしています。
...
auやソフトバンクでは「ウイルスメール規制」で受信するメールの添付ファイルに株式会社シマンテックのウイルスパターン情報を照合させて一致したら受信しなくなる機能です。受信のときだけ機能するためウイルスメール送信を防止することはできません。
...
=== シークレットコード登録（ドコモ）
この迷惑メールフィルターを使用している人は居ないとは思いますが、機能としてあるので紹介しておきます。ドコモの携帯電話でiモード契約をしていると「電話番号@docomo.ne.jp」が設定できますが（スマートフォン契約では電話番号をメールアドレスに設定はできません）、このメールのときだけ特別なルールが設定できます。
...
== 有料サービスや有料アプリケーションについて
@<b>{この項目は筆者の個人的な見解です。}

下に記載した迷惑メールフィルターと、@<hd>{chap07-allow-list|allow}を活用できれば有料サービスや有料アプリケーションは不要と考えますが、設定に自信がない方は導入の検討をオススメします。また、契約済みで効果が感じられないときは解約を検討してください。
