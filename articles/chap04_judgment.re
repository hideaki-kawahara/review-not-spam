...
判定を行う前に送信元を確認してください、@<b>{送信元に心当たりがないメールは迷惑メールです。}送信元の名前は詐称できるので確認するのは名前ではなく、送信元のメールアドレスまで確実に確認してください。このとき@<b>{メール本文は危険なので絶対に触らないでください}。
...
2020年現在、このような迷惑メールは非常に珍しいです。昔の迷惑メールではメール送信先を大量に並べて送信することがありましたが、今はこの形式でメールを送信するとほぼ迷惑メールとして判定され届かないので業者もこの形式を利用していないです。ときどき知識のない業者がこの形式でメール送信することがありますので、@<b>{知り合いからのメール以外では迷惑メールと判定して良い}でしょう。
...
undisclosed-recipients:;と表示されるメールの送信先を設定せずに送信すると表示されます。@<b>{undisclosed-recipients:;と表示されるメールは迷惑メールと判定して良い}です。メールマガジンなどでは送信先を設定するので、このような表示になることがないからです。このようなメール送信方法はメールソフトでは行うことができないので、知識のない業者がオペレーションミスをしたと推測できます。
...
迷惑メールを送信する業者は送信文面を確認せずに機械的に送信しています。そのため遠い昔に機械翻訳した「日本語が変」な状態のままで、メール内容を直すこともできない迷惑メールは知識のない迷惑メール業者に多いです。最近は文面もまともになったと言われていますが、日本語のホームページをそのままコピペしているケースが多く、送信元の名前などに@<b>{日本語漢字にそっくりな中国語を使用}していたりと、これらも落ち着いて迷惑メールを見ると見破れることが多いです。とにかく落ち着いて見ることをオススメします。
