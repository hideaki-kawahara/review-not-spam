...
それから銀行やクレジットカードや携帯電話会社は@<b>{画像を使ったHTMLメールを送信することはない}です。HTMLメールは見た目がわかりやすい特性がありますが、その反面URLが隠される問題があるので重要なお知らせはHTMLメールを使用しません。@<b>{HTMLメールを使用していたら迷惑メールとして判定して良いです。}

====[column] 赤い錠前が外れたアイコンについて

本当の銀行からのメールであっても、Gmailで確認すると下の図のように「赤い錠前が外れたアイコン」が表示されることがあります。
//image[risona][銀行からのメールで「赤い錠前が外れたアイコン」が表示][scale=0.5]{
//}
